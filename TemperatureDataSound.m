	...
	methods
		...
		function obj = UI_getPar(obj)
            ...
            lst_file = uicontrol('Parent',F,'Style','listbox','Units','Normalized',...
                'Position',[0.05 0.05 0.75 0.245],...
                'String',{'Load'},'tag','lst_file','fontsize',12,'fontweight','bold');
            
            btn_saveWork = uicontrol('Parent',F,'Style','pushbutton','Units','Normalized',...
        		'Position',[0.8 0.234 0.2 0.07], 'callback',@btn_save_work,...
        		'String','Save all','tag','btnSaveWork');
            
            btn_openWork = uicontrol('Parent',F,'Style','pushbutton','Units','Normalized',...
        		'Position',[0.8 0.150 0.2 0.07], 'callback',@btn_open_work,...
        		'String','Open','tag','btnOpenWork');
            ...
            function btn_save_work(src,evt)
                StorageTDS =  evalin('base','StorageTDS');
                uisave('StorageTDS');
            end
            
            function btn_open_work(src,evt)
                [n, p] = uigetfile('*.mat');
                S = load([p,n]);
                if isfield(S,'StorageTDS')
                    f = fields(S.StorageTDS);
                    for i = 1:length(f)
                        if isa(S.StorageTDS.(f{i}),'TemperatureDataSound')
                            assignin('base','StorageTDS',S.StorageTDS);
                            refresh_listbox(src,evt);
                            return;
                        end
                    end
...
%                     plot(Index(i),vFq2(Index(i)),'r*');
        ...
        function PlotByList(obj)
        	f = figure('DeleteFcn',@(src,evt)close_saving_PlotByList(src,evt));
            ...
            btnPlotFitX1X2_data = uicontrol('Parent',group3,'Style','pushbutton','Units','Normalized',...
                'Position',[0.05 0.05 0.9 0.1],'callback',@btn_Plot_yy_sin_fit,...
                'String','Fit All','tag','Plot_fitX1X2','fontsize',11,'fontweight','normal');
            btnPlotYY_data = uicontrol('Parent',group3,'Style','pushbutton','Units','Normalized',...
                'Position',[0.05 0.35 0.9 0.1],'callback',@btn_Plot_yy,...
...
%                 assignin('base',nameObj,obj);
            ...
            function btn_Plot_yy_sin_fit(src,evt)
                v = getTchb(); 
                curT = obj.getF('T',v(1));
                Angles = obj.getData('Angle',v(1));
                name = ppmGraph.String{ppmGraph.Value};
                
                Data1 = obj.getData(name,v(1),1);
                ind1 = find(Data1~=0);
                x1 = Angles(ind1);
                y1 = Data1(ind1)';
                
                Data2 = obj.getData(name,v(1),2);
                ind2 = find(Data2~=0);
                x2 = Angles(ind2);
                y2 = Data2(ind2)';
                ...
                Out1 = sin_fit(obj,x1,y1,per);
                xp1 = Out1{1}; 
                yp1 = Out1{2};
                max_x1 = Out1{3};
                
                Out2 = sin_fit(obj,x2,y2,per);
                xp2 = Out2{1}; 
                yp2 = Out2{2};
                max_x2 = Out2{3};
                ...
                subplot(211);
                title([name,' delta = ',num2str(abs(max_x2-max_x1))]);
                hold on;
                plot(x1,y1,'bo',  xp1,yp1, 'b:', 'linewidth',2);
                plot([max_x1,max_x1],get(gca,'YLim'),'--b','linewidth',2);
                text(max_x1,yp1(find(xp1==max_x1))+range(yp1)/10,['a1 = ',num2str(max_x1)],'Color','b','FontSize',12)
                set(gca,'XGrid','on','YGrid','on','GridAlpha',1);
                
                subplot(212);
                hold on;
                plot(x2,y2,'ro',  xp2,yp2, 'r:', 'linewidth',2);
                plot([max_x2,max_x2],get(gca,'YLim'),'--r','linewidth',2);
                text(max_x2,yp2(find(xp2==max_x2))+range(yp2)/10,['a2 = ',num2str(max_x2)],'Color','r','FontSize',12)
                set(gca,'XGrid','on','YGrid','on','GridAlpha',1);
            ...
            function btn_sine_fint(src,evt)
                ...
                per = 90;
                Out = sin_fit(obj,x,y,per);
                xp = Out{1}; 
                yp = Out{2};
                max_x = Out{3};
                figure;
                hold on;
                plot(x,y,'bo',  xp,yp, 'r:', 'linewidth',2);
                plot([max_x,max_x],get(gca,'YLim'),'--g','linewidth',2);
                set(gca,'XGrid','on','YGrid','on','GridAlpha',1);
                title(['period = ',num2str(per),' max = ',num2str(max_x)]);
            end
                        
            function close_saving_PlotByList(src,evt)
            	% save data in StorageTDS before closing
                vars = evalin('base','whos');
                for i= 1:length(vars)
                    if strcmp(vars(i).name,'StorageTDS')
                        StorageTDS =  evalin('base','StorageTDS');
                        fieldname = scan_data_name(obj,obj.path,'variable');
                        StorageTDS.(fieldname) = obj;
                        assignin('base','StorageTDS',StorageTDS);
                        return;
                    end
                end
        ...
        function Out = sin_fit(obj,x,y,per);
            % A = sin_fit(x,y,per);
            % Output Out = {xp, yp, max_x};
            yu = max(y);
            yl = min(y);
            yr = (yu-yl);                               % Range of ‘y’
            yz = y-yu+(yr/2);
